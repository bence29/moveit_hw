...
void pickAndPlace(moveit::planning_interface::MoveGroupInterface &move_group_interface,
  ...
  for(int i = 0; i < 5; i++){
    if(moveSuccess)
      ...
      moveSuccess = MoveToPose(box_pick_up_pose, move_group_interface, moveit_visual_tools, logger);
  }
  ...
  for(int i = 0; i < 5; i++){
    if(moveSuccess)
      ...
      moveSuccess = MoveToPose(put_down_pose, move_group_interface, moveit_visual_tools, logger);
  }
...
void palettazas_ompl(  //ez a "main"
